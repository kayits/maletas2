    ...
    <section>
        ...
        <div class="columns is-multiline">
            ...
            :key="mesa.mesa.idMesa">
                ...
                <div class="box">
                    <p class="title is-2 has-text-grey">Equipaje/Caja #{{ mesa.mesa.idMesa }}
                        <!-- <span class="title is-1 has-text-weight-bold is-pulled-right" v-if="mesa.mesa.total">
                            ${{ mesa.mesa.total }}
                        </span> -->
                    </p>
                    <p v-if="mesa.mesa.atiende">
                        <strong>Hecha por</strong>: {{ mesa.mesa.atiende }}
                    </p>
                    <p v-if="mesa.mesa.cliente">
                        <strong>Nombre </strong>: {{ mesa.mesa.cliente }}
                    </p>
                    <b-collapse 
                        class="card" 
                        animation="slide" 
                        ...
                        <div class="card-content">
                            <div class="content">
                                ...
                                :is-row-checkable="(row) => row.estado !== 'agregado'"
                                checkable
                                ...
                                :checkbox-type="checkboxType">
                                    ...
                                    <b-table-column field="estado" label="" v-slot="props">
                                        ...
                                        <b-icon icon="check" type="is-success" v-if="props.row.estado ==='Agregado'"></b-icon>
                                    </b-table-column>                                                                
                                </b-table>
                            </div>
                        </div>
                    ...
                    <div class="has-text-centered">
                        ...
                        <div class="field is-grouped is-centered" v-if="mesa.mesa.estado === 'ocupada'">
                            <p class="control">
                                <b-button type="is-success" @click="cobrar(mesa)">Cerrar Equipaje/Caja</b-button>
                            </p>
                            <p class="control">
                                <b-button type="is-info" icon-left="plus" @click="ocuparMesa(mesa)">Agregar  paquetes</b-button>
                            </p>
                            <p class="control">
                                <b-button type="is-warning" icon-left="check" v-if="checkedRows.length > 0" @click="marcarInsumosEntregados(mesa)">Marcar agregado</b-button>
                            </p>
                            <p class="control">
    ...
    methods:{
        cancelarOrden(id){
            this.$buefy.dialog.confirm({
                title: 'Cancelar Maleta/Caja ' + id,
                message: '¿Seguro deseas  cancelar?',
                    ...
                    .then(resultado => {
                        if(resultado){
                            this.$buefy.toast.open({
                                message: "Orden de Maleta/Caja " + id + " cancelada",
                                type: "is-success"
                            })
                            this.crearMesas()
                            this.cargando = false
        ...
        marcarInsumosEntregados(mesa){
            ...
            .then(resultado => {
                if(resultado){
                    this.$buefy.toast.open({
                        message: 'Paquetes marcados como entregados',
                        type: 'is-success'
                    })
                    this.crearMesas()
                    this.cargando = false
        ...
        cobrar(mesa){
            this.$buefy.dialog.prompt({
                title: `Confirmar Equipaje/Caja #` + mesa.mesa.idMesa,
                //message: `El cliente debe pagar $` + mesa.mesa.total,
                inputAttrs: {
                    type: 'number',
                    value:"1",
                    placeholder: 'Escribe la cantidad 1',
                    min: 1
                },
                trapFocus: true,
                onConfirm: (value) => {
